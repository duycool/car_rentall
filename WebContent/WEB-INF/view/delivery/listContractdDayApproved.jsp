...
<section id="main-content">
    ...
    <div>
      <table class="table" ui-jq="footable" ui-options='{
        ...
        <c:forEach items="${listContractday}" var="contractdayy">
        <tbody>
          <tr>
            <td>${contractdayy.id_car}</td>          
            <td>${contractdayy.custumerid}</td>
            <td>${contractdayy.email}</td>  
            <td>${contractdayy.deposit}</td>
            <td>${contractdayy.dayhire}</td>          
            <td>${contractdayy.payday}</td>
            <td>${contractdayy.totalday}</td>
            <td><img src="images/${contractdayy.avatar}" width="170px" height="220px"/></td>
            <td>${contractdayy.nameCar}</td> 
            <td>${contractdayy.totalMoney}</td>                    
             <td>            
             	<form action="deliveryContractday" method="post">
             			<input type="hidden" value="${contractdayy.speContractid}" name="speContractid"> 
             		<table border="0" width="35%" align="center">            
              	 <tr>
             	<td width="50%">ĐANGGIAO: </td>
                ...
                <td colspan="2" align="center"><input  class="btn btn-primary"  type="submit" value="submit"/></td>
             	...
             	<a href="addViolateDelivery?carid=${contractdayy.id_car}&custumerid=${contractdayy.custumerid}">Add Vi Phạm</a>            	         
             </td>                  
          </tr>
        </tbody>
        </c:forEach>
