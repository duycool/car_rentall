...
/**
 * Servlet implementation class UpdateCarManuManagerServlet
 */
@WebServlet(name = "updateCarManuManager", urlPatterns = { "/updateCarManuManager" })
public class UpdateCarManuManagerServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateCarManuManagerServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String manufacidStr = request.getParameter("manufact");
		int manufacid = Integer.parseInt(manufacidStr);
		Car car = new Car();
		car.setId_car(id);
		car.setManufacID(manufacid);
		
		CarDAO dao = new CarDAO();
		dao.updateCarManu(car);
		car = dao.getCar(id);
